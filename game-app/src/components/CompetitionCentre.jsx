...
function cupState(state,id){
 if(id==="UCL")return {outcome:state.ucl?.outcome||(!state.ucl?.clubs.some(c=>c.id===state.myClubId)?"NOT QUALIFIED":null),record:state.ucl?.campaignRecord};
 if(id==="UEL")return {outcome:state.uel?.outcome||(!state.uel?.clubs.some(c=>c.id===state.myClubId)?"NOT QUALIFIED":null),record:state.uel?.campaignRecord};
 return state.cupStatus?.[CUP_KEYS[id]]||{};
}
...
export function FixturesPanel({state}){
 ...
 const options=[league,...cupCompetitions(state.league),...(state.ucl?["UCL"]:[]),...(state.uel?["UEL"]:[])];
...
export function CupWorkspace({state,onOpen}){
 return <section className="cc-cups"><div className="cc-section-title"><span className="cc-eyebrow">COMPETITION CENTRE</span><h2>Your road to silverware</h2><p>Follow the draw, the next opponent and every step of your campaign.</p></div>{[...cupCompetitions(state.league),"UCL","UEL"].map(id=>{
  ...
  return <button className={`cc-cup-row ${ended?"is-ended":""}`} style={competitionTheme(id)} onClick={()=>onOpen(id)} key={id}>
   <div className="cc-cup-identity"><CompetitionMark id={id}/><div><span className="cc-eyebrow">{["UCL","UEL"].includes(id)?"EUROPEAN NIGHTS":"DOMESTIC SILVERWARE"}</span><h3>{competitionBrand(id).name}</h3><span className="cc-status-pill">{status.outcome|| (event?roundName(event):"Awaiting next draw")}</span></div></div>
...
function DomesticPath({state,events}){
 ...
 {tab==="path"&&(["UCL","UEL"].includes(competition)?((competition==="UCL"&&state.ucl?.knockoutBracket)?renderBracket?.():<div className="cc-empty"><Trophy size={32}/><h3>The knockout road opens after the league phase</h3><p>European results and the qualification picture update after every matchday.</p></div>):<DomesticPath state={state} events={events}/>)}
 </div></section></div>;
}
