...
function guestPlayers(id,base){return (REAL_GUEST_SQUADS[id]||"").split(";").filter(Boolean).map((row,index)=>{const [name,role]=row.split("|"),roll=seeded(`${id}:${name}`),age=18+Math.floor(roll*16),starterBonus=index<2?0:index<11?2:index<15?0:-2,ovr=Math.max(63,Math.min(86,Math.round(base+starterBonus+(roll-.5)*5))),potential=Math.max(ovr,Math.min(91,ovr+(age<21?7:age<24?4:age<28?2:0)));return {id:`eu-${id}-${index}`,slug:slug(name),name,role,group:ROLE_GROUP[role],age,ovr,potential,value:Math.max(1,Math.round((ovr-57)*(ovr-57)*.09)),stamina:70+Math.floor(seeded(`${id}:s:${name}`)*20),club:`eu-${id}`,number:index===0?1:index===1?12:index+1,condition:100,energy:100,confidence:0,appearances:0};});}
...
export function repairUclField(state){
 const u=state.ucl;if(!u||!Array.isArray(u.clubs)||!Array.isArray(u.rounds))return state;
 ...
 const guestById=new Map((state.europeanGuestClubs||[]).map(club=>[club.id,club]));
 const refreshed=u.clubs.map(club=>guestById.get(club.id)||club);
 if(u.phaseTable||u.knockoutBracket)return {...state,ucl:{...u,clubs:refreshed,clubIds:refreshed.map(club=>club.id)}};
 const wanted=selectUclField({...state,ucl:{...u,clubs:refreshed}});
 if(refreshed.length===36&&refreshed.some(club=>club.guest))return {...state,ucl:{...u,clubs:refreshed,clubIds:refreshed.map(club=>club.id)}};
 ...
 return clubs.length===36?{...state,ucl:{...u,clubs,clubIds:clubs.map(club=>club.id)}}:state;
}
